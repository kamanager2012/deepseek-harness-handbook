...
> Draft evidence snapshot: 2026-08-16. Every current-status statement carries `[待复核]`; this page does not replace the GitHub Release, Actions result, staging review, or installer smoke test.

## Three-layer version model

| Layer | Current value | Meaning |
|---|---|---|
| Codebase trunk | `[待复核]` | The current code/package line; do not infer it from the Stable tag |
| Stable Release | `v0.1.2` `[待复核]` | Latest Stable; Linux AppImage, macOS dmg, and Windows `DSH.Community.Setup.exe`, each with a `.sha256` sidecar |
| Preview Release | `[待复核]` | Not specified by this task; do not invent a Preview value |
...
Stable users should download [`v0.1.2`](https://github.com/kamanager2012/dsh-community/releases/tag/v0.1.2) `[待复核]`.
Do not promote an unreviewed Preview, main code line, or staging output as the Stable download.

The current `v0.1.2` assets are `[待复核]`:
...
|---|---|---|
| Normal `dsh-community` CI | `[待复核]` | Not independently asserted by this task |
| Release assets | `[待复核]` | `v0.1.2` has an AppImage, dmg, and Windows Setup, each with SHA256 |
| artifact-smoke | `[待复核]` | One Windows, macOS, and WSL/Linux clean-machine first-launch round is recorded |
| Official Runtime staging | `NOT_READY` `[待复核]` | The staging design is being redone; do not write “installer verified” |
| Three-OS release gate | `[待复核]` | Published assets and a smoke subset exist, but staging and the full user loop are not closed |
...
`v0.1.2` is the Latest Stable fact used by this draft `[待复核]`. The next goal is not
another version number: finish the staging redesign, recheck the exact artifact, and
retain full user-loop evidence.

## Stable release baseline vs current main

`v0.1.2` is the current Stable baseline `[待复核]`. The code/package line must not be
invented here, and a main-source smoke test or CI result cannot replace Release-asset and
staging review. Official Runtime staging is currently `NOT_READY`, so this draft must
not say that the installer has been verified.
...
| Official Runtime staging | `NOT_READY` `[待复核]` | The staging design is being redone; do not claim the installer is verified |
...
machine first-launch checks `[待复核]`, but the `NOT_READY` staging state means the draft
must not call the installer verified.
...
| Phase 4 · Distribution Reality Gate | `[待复核]` | One Win/mac/Linux first-launch smoke round is recorded, but staging is NOT_READY and the full loop is open |
