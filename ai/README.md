...
这里是给 AI 检索和工具调用使用的静态知识包，不是第二套事实来源。

## 它解决什么问题

普通 Markdown 适合人连续阅读，但 AI 通常只需要某个具体问题的一小段内容。知识包把 `content/` 下的正文按 Markdown 的二级标题拆成检索记录，每条记录都带有：
...
不要手工修改 `catalog.jsonl` 和 `manifest.json`。修改正文后运行：
